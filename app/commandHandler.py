...
class Handler:
    def __init__(self):
        ...
        self.longpoll = VkBotLongPoll(vk=self.vk_session, group_id=settings.GROUP_ID)
...
class keyboardHandler(Handler):
    ...
    def get_required_keyboard(self, keyboard:str, event: Optional[VkBotEvent] = None) -> int:
        return vkKeyboard.required_keyboard(self.vk, self.session, keyboard, event)

    def info(self, event: Optional[VkBotEvent] = None) -> int:
        return vkKeyboard.info(self.vk, self.session, event)

    def problem(self, event: Optional[VkBotEvent] = None) -> int:
        return vkKeyboard.problem(self.vk, self.session, event, self.longpoll)
